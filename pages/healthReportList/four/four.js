...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.request({
      ...
      success(res) {
        ...
        // 获取糖尿病体质检测表
        wx.request({
          url: 'https://api.jiankangyouyi.com/ego-gw/v4/evaluation/content/load.do?evaluationCode=GXZPC',
          method: 'POST',
          header: {
            'token': token
          },
  ...
  submit() {
    ...
    wx.request({
      ...
      success(res) {
        ...
        // 解析测评问卷
        wx.request({
          url: 'https://api.jiankangyouyi.com/ego-gw/v4/evaluation/content/analysis.do?evaluationCode=GXZPC',
          method: 'POST',
          data: {
            "nonceStr": "5cbef2212f6264792914e081",
            "appId": "61527410837fee1bc97b6f43",
          ...
          success(res) {
            ...
            wx.request({
              ...
              success(res) {
                var token = res.data.data.token;
                wx.request({
                  url: 'https://api.jiankangyouyi.com/ego-gw/v4/evaluation/report/generate.do?evaluationCode=GXZPC',
                  method: 'POST',
                  header: {
                    // 'Content-Type': 'application/json',
                    // 'Accept':'application/json;charset=utf-8',
                  ...
                  success(res) {
                    ...
                    // 生成测评报告
                    wx.request({
                      url: 'https://api.jiankangyouyi.com/ego-gw/v4/evaluation/report/load.do?evaluationCode=GXZPC',
                      method: 'POST',
                      header: {
                        // 'Content-Type': 'application/json',
                        // 'Accept':'application/json;charset=utf-8',
  ...
  tap(e) {
    ...
    // }
    var array = [];
    array.push(this.data.one, this.data.two, this.data.three, this.data.four, this.data.five, this.data.six, this.data.seven);
    this.setData({
      newList: array
    })
    console.log("西医选项是======")
